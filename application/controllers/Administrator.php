...
class Administrator extends CI_Controller {
	...
	function delete_manajemenmodul(){
     ...
     // GUDANG MODUL ==============================================================================================================================================================================
        ...
        // Controller Modul Lokasi

        function lokasigudang(){
            cek_session_akses('lokasigudang',$this->session->id_session);
            $data['record'] = $this->model_app->view_ordering('rb_gudang','id_gudang','DESC');
            $this->template->load('administrator/template','administrator/gudang/mod_lokasi/view_lokasi',$data);
        }
        ...
        function tambah_lokasi(){
            ...
            if (isset($_POST['submit'])){
                $data = array('nama'=>$this->input->post('nm'),
                            'batas_kapasitas'=>$this->input->post('bk'),
                            'biaya_sewa'=>$this->input->post('bs'),
                            'gratis_biaya'=>$this->input->post('gb'),
                            'provinsi'=>$this->input->post('pv'),
                            'kota'=>$this->input->post('ko'),
                            'kecamatan'=>$this->input->post('ke'));

                $this->model_app->insert('rb_gudang',$data);
                redirect('administrator/lokasigudang');
            }else{
                $data['negara'] = $this->model_app->view_ordering('rb_provinsi','provinsi_id','DESC');
                $this->template->load('administrator/template','administrator/gudang/mod_lokasi/view_lokasi_tambah',$data);
            }
        }
        ...
        function delete_lokasi(){
            cek_session_akses('lokasigudang',$this->session->id_session);
            $id = array('id_gudang' => $this->uri->segment(3));
            $this->model_app->delete('rb_gudang',$id);
            redirect('administrator/lokasigudang');
        }
    ...
    // Controller Modul Brand Produk

    function brand_produk(){
        cek_session_akses('brand_produk',$this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('rb_brand_produk','id_brand_produk','DESC');
        $this->template->load('administrator/template','administrator/additional/mod_brand_produk/view_brand_produk',$data);
    }

    function tambah_brand_produk(){
        ...
        if (isset($_POST['submit'])){
            ...
            for($i=0; $i<$cpt; $i++){
                ...
                $this->upload->initialize($this->set_upload_brands());
            ...
            $data = array(
                'nama_brand'    => $this->input->post('a'),
                'brand_seo'     => seo_title($this->input->post('a')),
                'gambar_brand'  => trim($fileName),
        //        'brand_status'  => $this->input->post('s')
            );
            if(trim($fileName)==''){
                unset($data['gambar_brand']);
            }
            
            $this->model_app->insert('rb_brand_produk',$data);
            redirect('administrator/brand_produk');
        }else{
            $this->template->load('administrator/template','administrator/additional/mod_brand_produk/view_brand_produk_tambah');
        }
    }

    function edit_brand_produk(){
        cek_session_akses('brand_produk',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $files = $_FILES;
        ...
        //        'brand_status'  => $this->input->post('s')
            ...
            $where = array('id_brand_produk' => $this->input->post('id'));
            $this->model_app->update('rb_brand_produk', $data, $where);
            redirect('administrator/brand_produk');
        }else{
            $edit = $this->model_app->edit('rb_brand_produk',array('id_brand_produk'=>$id))->row_array();
            $data = array('rows' => $edit);
            $this->template->load('administrator/template','administrator/additional/mod_brand_produk/view_brand_produk_edit',$data);
        }
    }

    function delete_brand_produk(){
        cek_session_akses('brand_produk',$this->session->id_session);
        $id = array('id_brand_produk' => $this->uri->segment(3));
        $this->model_app->delete('rb_brand_produk',$id);
        redirect('administrator/brand_produk');
    }

    private function set_upload_brands(){
        $config = array();
        $config['upload_path'] = 'asset/foto_brand/';
        $config['allowed_types'] = 'gif|jpg|png|jpeg';
        $config['max_size'] = '2000'; // kb
    ...
    function tambah_produk(){
        ...
        if (isset($_POST['submit'])){
            ...
            if (trim($fileName)!=''){
                $data = array(
                              'id_brand_produk'=>$this->input->post('br'),
                ...
                $data = array('id_brand_produk'=>$this->input->post('br'),
                              'id_kategori_produk'=>$this->input->post('a'),
                              'id_kategori_produk_sub'=>$this->input->post('aa'),
                              'nama_produk'=>$this->input->post('b'),
                              'produk_seo'=>seo_title($this->input->post('b')),
            ...
            $data['record'] = $this->model_app->view_ordering('rb_kategori_produk','id_kategori_produk','DESC');
            $data['brands'] = $this->model_app->view_ordering('rb_brand_produk','id_brand_produk','DESC');
